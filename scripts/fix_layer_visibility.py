...
def fix_layers_in_dxf(input_path, output_path):
    ...
    while i < len(lines):
        ...
        # Detect end of a LAYER entry (next entity or end table)
        if in_layer_entry and line == '0' and lines[i-1].strip() not in ('2', '5', '6', '62', '70', '330', '100', '347', '348', '370', '390', '1000', '1001'):
            # If the previous line was a group code that starts a new entity
            if i + 1 < len(lines) and (lines[i+1].strip() in ('LAYER', 'ENDTAB')):
                in_layer_entry = False
        ...
        # Also catch the ENDTAB / next entity boundary properly
